...
# ===================== CONFIG =====================
BOT_TOKEN = "8997395334:AAES-YSzGhG1IpCH-5XsxD8tP8D3Ffq4k0o"
JOIN_CHANNEL = "@Trendnexushub12"
SUPPORT_USERNAME = "@ImtiajHossain15"
SUPPORT_USERNAME_MD = SUPPORT_USERNAME  # no underscore, no escaping needed
BOT_USERNAME = "ABSMMPanelRobot"
ADMIN_ID = 7237820659          # Primary admin
ADMIN_IDS = {7237820659, 6520878121}  # Both admins — use is_admin() for all checks
...
GRAM_WALLET_ADDRESS = "UQDH1bNydAXsVMO9z9cBCf8mDwQXNHc6GNco2sOlWMkSrfby"   # TON wallet (GRAM)
...
TONCENTER_API_KEY = "ac7ee79647cb86a54f4339ecfc8aee94a3aad549b67a0c7e6df73a97486bd851"
...
PROVIDER_API_KEY = "dc6d9a49b44ffe66f45114cf52a1bb06"
    ...
    for d in candidates:
        try:
            ...
            with open(test, "w") as f:
                f.write("ok")
            os.remove(test)
            path = os.path.join(d, "wallet_data.json")
            print(f"✅ Data file: {path}")
            return path
        except Exception as e:
            print(f"⚠️ {d} not writable: {e}")
    raise RuntimeError("❌ No writable directory found!")

DATA_FILE: str = _resolve_data_file()
DATA_DIR: str = os.path.dirname(DATA_FILE)
DATA_DIR_WRITABLE: bool = True
...
def load_data():
    for filepath in [DATA_FILE, DATA_FILE + ".bak"]:
        try:
            ...
            print(f"⚠️ load_data failed for {filepath}: {e}")
            continue
...
def get_custom_services(platform: str = None) -> list:
    with _DATA_LOCK:
        ...
        if platform:
            platform = platform.strip().lower()
            return [s for s in services if s.get("platform", "").strip().lower() == platform]
        return services
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_back":
        ...
        if not services:
            ...
            text = "\n".join(lines)  # ← was missing, caused UnboundLocalError
        kb = InlineKeyboardMarkup([
        ...
        if total == 0:
            ...
            for uid, u in items[:70]:
                ...
                # escape underscores so usernames like @some_name don't break Markdown
                uname_str = f"@{uname.replace('_', chr(92) + '_')}" if uname else "—"
                fname = (u.get("first_name") or "—").replace("_", "\\_").replace("*", "\\*").replace("`", "\\`")
...
async def global_error_handler(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Suppress known harmless / high-frequency errors that should never spam admin
    ...
    if "terminated by other getupdates" in err_text or "conflict" in err_text:
        # Another instance is running with the same token — log it but do NOT
        # spam the admin with repeated messages every few seconds.
        print(f"⚠️ Conflict error (another instance running?): {context.error}")
        return

    # Send the real error to both admins so we can debug without Railway logs
    user_info = ""
    if update and hasattr(update, "effective_user") and update.effective_user:
        user_info = f"User: `{update.effective_user.id}`\nAction: `{update.callback_query.data if update.callback_query else 'message'}`\n\n"
    error_msg = f"🚨 *Bot Error*\n\n{user_info}`{type(context.error).__name__}: {context.error}`\n\n```{tb[-800:]}```"
    for aid in ADMIN_IDS:
        try:
            await context.bot.send_message(chat_id=aid, text=error_msg, parse_mode=ParseMode.MARKDOWN)
        except Exception:
            pass


async def _post_init_storage_check(app):
    """Runs once the bot starts. Deletes any existing webhook (clears Conflict errors
    caused by a previous webhook or another polling instance), then confirms /data
    (the Volume) is writable so deposits/balances actually persist."""
    # Delete webhook first — if a webhook was ever set (even accidentally), Telegram
    # will reject polling with a Conflict error forever until this is cleared.
    try:
        await app.bot.delete_webhook(drop_pending_updates=True)
        print("✅ Webhook cleared — polling mode active.")
    except Exception as e:
        print(f"⚠️ delete_webhook failed (non-fatal): {e}")
    ...
    if DATA_DIR_WRITABLE:
        ...
        try:
            for aid in ADMIN_IDS:
                try:
                    await app.bot.send_message(chat_id=aid, text=warning, parse_mode=ParseMode.MARKDOWN)
                except Exception:
                    pass
        except Exception as e:
...
def main():
    ...
    # Force-clear any webhook BEFORE building the app — this is the #1 cause of
    # Conflict errors on Railway. If a webhook was ever set (even by accident or
    # a previous deploy), Telegram blocks polling forever until it's deleted.
    try:
        r = _httpx.post(
